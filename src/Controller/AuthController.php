...
class AuthController extends AppController {
    ...
    public function initialize(): void {
        ...
        $this->Authentication->allowUnauthenticated(['login', 'register', 'forgetPassword', 'resetPassword', 'test']);
            ...
            if ($user) {
                ...
                if ($this->Users->save($user)) {
                    ...
                    $mailer
                        ->setEmailFormat('both')
                        ->setTo($user->email)
                        ->setSubject('Gemino Account - Password Reset');
                    ...
                    if (!$mailer->deliver()) {
                        ...
                        $this->Flash->error('We have encountered an issue sending you an email. Please try again.');
                    ...
                    $this->Flash->error('We have encountered an issue resetting your password. Please try again.');
                    return $this->render();  // Skip the rest of the controller and render the view
                }
            }
            /*
    ...
    public function resetPassword($nonce = null)
    {
        ...
        // If nonce cannot find the user, or nonce is expired, prompt for reset password
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Users->save($user)) {
                $this->Flash->success(__('Your password has been successfully reset. Please login with your new password.'));
            }
            else {
                $this->Flash->success(__('An error occurred. Please try again.'));
            }
            return $this->redirect(['action' => 'login']);
        }
        $this->set(compact('user'));
